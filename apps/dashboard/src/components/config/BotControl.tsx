...
export function BotControl({ guildId, bots, online, channels }: { guildId: string; bots: GuildBot[] | null; online: boolean; channels: { id: string; name: string }[] }) {
  ...
  const { selectedBot } = useBotSelection();
  useEffect(() => {
    const timer = setInterval(() => { if (document.visibilityState === "visible") router.refresh(); }, 30_000);
    return () => clearInterval(timer);
  }, [router]);
  return <div className="flex flex-col gap-6">
    <SectionCard title="Centre de contrôle" description="Choisis un bot dans la barre de gauche, puis modifie sa présence, son profil ou ses messages."
      aside={<span className={`pill ${online ? "pill-ok" : "pill-danger"}`}>{online ? "Service disponible" : "Indisponible"}</span>}>
    ...
    <SectionCard title={selectedBot?.name ?? "Bot sélectionné"} description="Les réglages de présence sont conservés après un redémarrage.">
      {bots === null ? <p role="alert" className="text-sm text-muted">Inventaire indisponible. Vérifie la connexion du bot et son intent Server Members, puis actualise.</p>
        : bots.length === 0 ? <p>Aucun bot trouvé.</p> : <div className="flex flex-col gap-4">
          {bots.filter((bot) => bot.id === selectedBot?.id).map((bot) => <ManagedBotCard key={bot.id} bot={bot} guildId={guildId} channels={channels} />)}
        </div>}
    </SectionCard>
  </div>;
}
