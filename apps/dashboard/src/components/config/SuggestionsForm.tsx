...
export function SuggestionsForm({ guildId, forums, initial }: { guildId: string; forums: { id: string; name: string }[]; initial: { enabled: boolean; channelId: string; botId: string | null } }) {
  const [enabled, setEnabled] = useState(initial.enabled);
  const [channelId, setChannelId] = useState(initial.channelId);
  const [pending, startTransition] = useTransition();
  const [msg, setMsg] = useState<{ ok: boolean; message: string } | null>(null);
  const { selectedBotId } = useBotSelection();
  return <div className="flex flex-col gap-6"><BotPicker assignedBotId={initial.botId} label="Bot qui ajoute les votes" /><SectionCard title="Votes automatiques" description="Le bot choisi doit pouvoir voir le forum, lire les messages et ajouter des réactions." aside={<Toggle checked={enabled} onChange={setEnabled} />}>
    ...
    {!forums.length && <p className="mt-3 text-sm text-muted">Aucun forum disponible. Vérifie les salons et la connexion du centre de contrôle.</p>}
  </SectionCard><SaveBar pending={pending} msg={msg} onSave={() => startTransition(async () => {
    try { setMsg(await saveSuggestionsAction(guildId, { enabled, channelId, botId: selectedBotId || null })); }
